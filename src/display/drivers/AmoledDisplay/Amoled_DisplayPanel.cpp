...
// SensorLib 0.4 replaced getPoint() with a TouchPoints snapshot; copy out at most max_points coordinates.
uint8_t Amoled_DisplayPanel::readTouchPoints(int16_t *x_array, int16_t *y_array, uint8_t max_points) {
    const TouchPoints points = _touchDrv->getTouchPoints();
    const uint8_t touched = std::min<uint8_t>(points.getPointCount(), max_points);
...
uint8_t Amoled_DisplayPanel::getPoint(int16_t *x_array, int16_t *y_array, uint8_t get_point) {
    if (touchType == TOUCH_CST92XX) {
        return readTouchPoints(x_array, y_array, _touchDrv->getSupportTouchPoint());
    }
    ...
    uint8_t points = readTouchPoints(x_array, y_array, get_point);
