...
Preact + Vite + Tailwind 4 / daisyUI, signals for state. `services/ApiService.js` owns the WebSocket to `/ws` (with reconnect/backoff) and exposes a `machine` signal; pages under `src/pages/`. All messages are JSON with a `tp` field — `req:` from client, `res:`/`evt:` from device. The contract is documented in `docs/websocket-api.yaml` (AsyncAPI); keep it in sync when adding a message type. Every `req:*` command also has an HTTP equivalent (`POST /api/mode`, `/api/process/{action}`, `/api/targets/...`, `/api/ota`, `/api/history/rebuild`, `/api/profiles/reorder`, …) that calls the same controller code — use those from scripts and tests; the socket is for `evt:*` pushes. `sim/tests/test-machine-api.sh` exercises them against the simulator. The HTTP surface (settings partial update, REST profile CRUD, shot history downloads) is documented in `docs/http-api.yaml` (OpenAPI) — same rule. `sim/tests/` holds curl-based end-to-end suites for the HTTP API that run against the simulator. Bulk data (shot history index, `.slog` files) goes over plain HTTP under `/api/history/`.
