...
descriptor on the mount path) rather than the Arduino `File` wrapper: the wrapper is
`fopen`, and newlib gives each open FILE a 4 KB internal stdio buffer, which was the largest
allocation live during a shot (heap trace, 2026-09-05). Keep new code on the shot path off
`fs->open`; the simulator maps RawFile's paths through `FS::hostPath`.
