...
├── eez-ui/             # EEZ Studio project for the LVGL UI (exported to src/display/ui/default/eez/)
...
Uses LVGL for graphics, with UI assets generated by EEZ Studio from `eez-ui/gaggimate.eez-project` into `src/display/ui/default/eez/` (regenerate, don't hand-edit).
...
`scripts/build_webui.sh` installs the web dependencies (`npm ci`), builds the app, gzips the assets and runs `scripts/embed_webui.py`, which packs them into `src/display/webassets/` (git-ignored) for embedding into the display firmware's flash. Run it before `platformio run -e display`; without it the firmware still links, but with an empty stub in place of the web UI.
...
2. **Install dependencies**:
   - Run `npm ci` inside the `web/` directory for the web interface (the build script does this too).
   - Ensure [PlatformIO](https://platformio.org/install/cli) is available for firmware builds.

3. **Build the project** to verify your environment — the web UI first, so the display image embeds it rather than an empty stub:
